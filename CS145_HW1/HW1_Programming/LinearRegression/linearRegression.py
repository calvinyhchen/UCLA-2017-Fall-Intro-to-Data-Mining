...
def getBeta(train_x, train_y):
    ...
    beta = inv(np.dot(train_x.transpose(),train_x))
    beta = np.dot(beta,train_x.transpose())
...
def getBetaBatchGradient(train_x, train_y, alpha):
    ...
    ########## Please Fill Missing Lines Here ##########
    #print np.dot(  train_x[1].transpose() , (np.dot(train_x[1],beta) - train_y[1]) )    
    flag = 0
    while 1: 
        flag+=1 
        beta_2 = beta - alpha*np.dot(  train_x.transpose() , (np.dot(train_x,beta) - train_y) )
        if( ((np.dot(train_x,beta) - train_y)**2).mean() == ((np.dot(train_x,beta_2) - train_y)**2).mean() ):
            print 'Step: ',flag
            return beta_2
        #print ((np.dot(train_x,beta) - train_y)**2).mean()    
        beta = beta_2
    return beta
...
def getBetaStochasticGradient(train_x, train_y, alpha):
    ...
    while 1:
        ...
        for i in range(0,1000):
            beta_2 = beta_2 + alpha*np.dot(  train_x[i].transpose() , (-np.dot(train_x[i],beta) + train_y[i]) )

        if( int(1000000*((np.dot(train_x,beta) - train_y)**2).mean()) == int(1000000*((np.dot(train_x,beta_2) - train_y)**2).mean()) ):
            print 'Step: ',flag
            return beta_2  
        #print ((np.dot(train_x,beta) - train_y)**2).mean()  
        beta = beta_2    
