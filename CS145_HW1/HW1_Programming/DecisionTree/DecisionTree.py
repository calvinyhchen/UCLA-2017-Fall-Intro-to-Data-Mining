...
def compute_entropy(data):
    ...
    #for key in data:
        #total_entropy -= data[key].num_instances
        #print(data[key].num_instances)
    if data.num_instances == 0:
        return total_entropy

    dict = {data.label[0]:0}
    for i in range(0,data.num_instances): 
        if data.label[i] in dict:
            dict[data.label[i]] += 1
        else:
            dict[data.label[i]] = 1
    for i in dict:
        p = dict[i]/data.num_instances;
        if p != 0:
...
def compute_info_gain(data, att_idx):
    ...
    for i in dict:
        p = dict[i]/data.num_instances;
        if p != 0:
            info_gain -= p*math.log(p,2) 

    sub_data = data.split(att_idx)
    for key in sub_data :
        #print(sub_data[key].num_instances)
        info_gain -= sub_data[key].num_instances/data.num_instances * compute_entropy(sub_data[key])
    #print (info_gain)
    return info_gain
...
def comput_gain_ratio(data, att_idx):
    ...
    split_info = 0
    for key in sub_data :
        p = sub_data[key].num_instances/data.num_instances
        if p != 0:
            split_info -= p*math.log(p,2)
    if split_info==0:
        return gain_ratio
    gain_ratio = compute_info_gain(data,att_idx) / split_info
    #print (gain_ratio)
    return gain_ratio
...
class DecisionTree(object):
    ...
    def make_tree(self):
        if self.instances.num_instances == 0:
            ...
            #print (self.m_attr_idx)
            if np.abs(gains[self.m_attr_idx]) < 1e-9:
                # A leaf to decide the decided class
                self.m_attr_idx = None
                ########## Please Fill Missing Lines Here ##########
                
                dict = {self.instances.label[0]:0}
                for i in range(0,self.instances.num_instances): 
                    if self.instances.label[i] in dict:
                        dict[self.instances.label[i]] += 1
                    else:
                        dict[self.instances.label[i]] = 1
