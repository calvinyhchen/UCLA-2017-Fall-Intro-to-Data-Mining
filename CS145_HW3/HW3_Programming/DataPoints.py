...
class DataPoints:
    ...
    def getMean(clusters, mean):
        ...
        for set_j in range(len(mean)):
            for point_i in clusters[set_j]:
                mean[set_j][0] += point_i.x
                mean[set_j][1] += point_i.y
            mean[set_j][0] /= len(clusters[set_j])
            mean[set_j][1] /= len(clusters[set_j])
        pass
    # -------------------------------------------------------------------
    @staticmethod
    def getStdDeviation(clusters, mean, stddev):
        ...
        # ****************Please Fill Missing Lines Here*****************
        for set_j in range(len(stddev)):
            for point_i in clusters[set_j]:
                stddev[set_j][0] += (point_i.x-mean[set_j][0])**2
                stddev[set_j][1] += (point_i.y-mean[set_j][1])**2
            stddev[set_j][0] /= len(clusters[set_j])
            stddev[set_j][1] /= len(clusters[set_j])
            stddev[set_j][0] = math.sqrt(stddev[set_j][0])
            stddev[set_j][1] = math.sqrt(stddev[set_j][1])
    ...
    def getCovariance(clusters, mean, stddev, cov):
        ...
        # ****************Please Fill Missing Lines Here*****************
        for set_j in range(len(stddev)):
            cov[set_j][0][0] = stddev[set_j][0]**2
            cov[set_j][1][1] = stddev[set_j][1]**2
            for point_i in clusters[set_j]:
                cov[set_j][0][1] += (point_i.x-mean[set_j][0])*(point_i.y-mean[set_j][1])
            cov[set_j][0][1] /= len(clusters[set_j])
            cov[set_j][1][0] = cov[set_j][0][1]    
