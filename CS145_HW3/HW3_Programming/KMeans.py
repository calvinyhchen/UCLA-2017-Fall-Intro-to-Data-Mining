...
class KMeans:
    ...
    def main(self, args):
        ...
        self.kmeans(dataSet,"KMeans_1.png")

        print("")
        dataSet = self.readDataSet("dataset2.txt")
        self.K = DataPoints.getNoOFLabels(dataSet)
        random.Random(seed).shuffle(dataSet)
        self.kmeans(dataSet,"KMeans_2.png")
        
        print("")
        dataSet = self.readDataSet("dataset3.txt")
        self.K = DataPoints.getNoOFLabels(dataSet)
        random.Random(seed).shuffle(dataSet)
        self.kmeans(dataSet,"KMeans_3.png")
    # -------------------------------------------------------------------
    def kmeans(self, dataSet, s):
        clusters = []
        k = 0
        while k < self.K:
            cluster = set()
        ...
        for w in range(self.K):
            ...
            for point in clusters[w]:
                ...
                plt.scatter(point.x, point.y, c=LABEL_COLOR_MAP[w])
        f.close()
        plt.savefig(s,dpi=300,format="png") 
        plt.gcf().clear()
